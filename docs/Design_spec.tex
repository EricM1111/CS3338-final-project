...
\begin{itemize}[noitemsep]
    \item Complete frontend image upload interface with image preview
    \item Backend API with image validation and error handling
    \item OpenCV-based copy-move forgery detection
    \item Refined results display with marked suspicious regions
    \item Containerized deployment using Docker Compose
\end{itemize}
...
\textbf{Services:}
\begin{itemize}[noitemsep]
    \item Frontend container (nginx serving HTML)
    \item Backend container (FastAPI + Uvicorn)
    \item Database container (PostgreSQL --- planned for future use)
\end{itemize}
...
\begin{enumerate}[noitemsep]
    \item User uploads biomedical image through frontend interface
    \item Frontend shows image preview and sends to backend via POST request
    \item Backend validates image file format
    \item If invalid, backend returns error message
    \item If valid, detection module analyzes image for duplicated regions
    \item Suspicious regions identified and marked on output image
    \item JSON response returned to frontend with results
    \item Frontend displays marked image and status message
\end{enumerate}
...
Keypoint & A distinctive location in an image used for matching \\[0.5em]
Docker & Platform for containerizing applications \\[0.5em]
Docker Compose & Tool for defining multi-container Docker applications \\[0.5em]
...
1.2 & May 12, 2026 & Added fraudulent region detection workflow and frontend/backend integration updates & Janelle Rivera, Eric Marroquin, Alexis Flores, Alex Lam \\[0.5em]
